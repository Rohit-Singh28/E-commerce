...
const CategoryOpt = () => {
    ...
    const fetchData = async () => {
        setLoader(true);
        const response = await axios.get(`${BackendURL}/api/product/distinct`);
        setLoader(false);
    ...
    return (
        <div className='w-full'>
            <div className='  flex items-center gap-4 justify-between overflow-x-scroll scrollbar-none px-0 md:px-4 '>
                {
                    loader ? (
                            ...
                            return (
                                <Link to={`/${product.category}`} key={uuidv4()}>
                                    <div className='flex flex-col'  >
                                        <div className='w-16 h-16 md:w-20 md:h-20 rounded-full overflow-hidden p-1 md:p-4 bg-slate-200 flex items-center justify-center'>
